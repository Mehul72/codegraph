...
import { MAX_SIGNATURE, cleanDoc, endLineOf, field, fieldText, firstChildOfType, leadingCommentDoc, lineOf, looksLikeConstant, namedChildren, signatureOf, splitQualified, walk, } from './ast.js';
...
function tsExtractor(id, extensions, grammar) {
    return {
        id,
        extensions,
        grammar,
...
export const javascriptExtractor = tsExtractor('javascript', ['.js', '.mjs', '.cjs', '.jsx'], 'tree-sitter-javascript.wasm');
function extractModule(input, lang) {
    ...
    try {
        ...
        log.debug(`${filePath}: extraction stopped early (${err.message})`);
    }
    return build.result();
}
...
function visitNode(node, scope) {
    switch (node.type) {
        ...
        case 'template_string':
            ...
            return;
        default:
            if (!defineDeclaration(node, scope, false))
                visitChildren(node, scope);
            return;
    }
}
 ...
 */
function defineDeclaration(node, scope, exported) {
    switch (node.type) {
 ...
 * encloses it.
 */
function declareSymbol(node, scope, spec) {
...
function defineFunction(node, scope, exported, fallbackName) {
...
function defineClass(node, scope, exported, fallbackName) {
    const name = fieldText(node, 'name') ?? fallbackName;
    if (!name)
        return;
    const qualified = qualify(name, scope);
    ...
    if (body)
        visitMembers(body, symbol, qualified, inner);
}
function defineInterface(node, scope, exported) {
...
function defineTypeAlias(node, scope, exported) {
    const name = fieldText(node, 'name');
    if (!name)
        return;
    const symbol = declareSymbol(node, scope, {
    ...
    if (value)
        recordTypeRefs(value, symbol, withTypeParams(node, scope));
}
/** An enum is a named set of values with a type, which is close enough to a class. */
function defineEnum(node, scope, exported) {
    const name = fieldText(node, 'name');
    if (!name)
        return;
 ...
 */
function recordVariables(node, scope, exported) {
    ...
    for (const declarator of namedChildren(node)) {
        ...
        if (name === null || !scope.moduleScope || !(callable || looksLikeConstant(name))) {
            if (annotation)
                recordTypeRefs(annotation, scope.owner, scope);
            if (value)
        ...
        if (annotation)
            recordTypeRefs(annotation, symbol, scope);
        if (!value)
            continue;
        if (callable)
            visitCallable(value, symbol, qualified, scope);
        else
            visitNode(value, { ...scope, owner: symbol });
    }
}
function recordExport(node, scope) {
    ...
    if (declaration) {
        ...
        if (!defineDeclaration(declaration, scope, true))
            visitChildren(declaration, scope);
        return;
    }
    ...
    if (FUNCTION_VALUES.has(value.type))
        defineFunction(value, scope, true, 'default');
    else if (value.type === 'class')
        defineClass(value, scope, true, 'default');
    else
        visitNode(value, scope);
}
 ...
 */
function recordReexport(node, source, scope) {
    const module = unquote(source.text);
    const line = lineOf(node);
...
function recordCall(node, scope) {
    ...
    if (typeArgs)
        recordTypeRefs(typeArgs, scope.owner, scope);
    ...
    if (target.type === 'member_expression') {
        ...
        if (ROUTE_METHODS.has(property.toLowerCase()))
            defineEndpoint(node, property, scope, line);
        return;
    }
...
function recordNew(node, scope) {
    ...
    if (IDENTIFIER.test(name))
        scope.build.ref(scope.owner, 'calls', name, lineOf(node), qualifier);
}
/** `require('./m')` and dynamic `import('./m')`, when the path is a literal. */
function recordModuleArgument(node, scope, line) {
 ...
 */
function defineEndpoint(callNode, methodName, scope, line) {
 ...
 */
function recordHeritage(node, symbol, scope) {
    const heritage = firstChildOfType(node, 'class_heritage');
    if (!heritage)
        return;
    for (const child of namedChildren(heritage)) {
        if (child.type === 'implements_clause') {
            for (const base of namedChildren(child))
                recordSuperType(base, symbol, 'implements', scope);
            continue;
        }
...
function recordSuperType(node, symbol, type, scope) {
    ...
    if (IDENTIFIER.test(name))
        scope.build.ref(symbol, type, name, lineOf(node), qualifier);
}
function visitMembers(body, container, prefix, scope) {
    const inner = { ...scope, owner: container, prefix, moduleScope: false };
 ...
 */
function recordMemberField(node, scope) {
    const name = memberName(node);
    const value = field(node, 'value');
    const annotation = field(node, 'type');
    const callable = (value !== null && FUNCTION_VALUES.has(value.type)) || (annotation !== null && holdsFunctionType(annotation));
    if (!name || !callable) {
        if (annotation)
            recordTypeRefs(annotation, scope.owner, scope);
        if (value)
 ...
 */
function visitCallable(node, symbol, qualified, scope) {
    const inner = { ...withTypeParams(node, scope), owner: symbol, prefix: qualified, moduleScope: false };
    // A type parameter constraint (`<T extends Widget>`) is a dependency too.
    for (const part of ['type_parameters', 'parameters', 'return_type']) {
        const child = field(node, part);
        if (child)
            recordTypeRefs(child, symbol, inner);
    }
    const body = field(node, 'body');
    if (body)
        visitNode(body, inner);
}
/**
 ...
 */
function withTypeParams(node, scope) {
 ...
 */
function fileDoc(root, source) {
    const children = namedChildren(root);
    const header = children[0];
    if (header?.type !== 'comment' || header.startIndex !== 0)
        return null;
    const next = children[1];
    if (next && !/\n[ \t]*\n/.test(source.slice(header.endIndex, next.startIndex)))
        return null;
...
function publicNames(root) {
    ...
    for (const child of namedChildren(root)) {
        ...
        if (child.type !== 'export_statement' || field(child, 'source'))
            continue;
 ...
 */
function collectCommonJsExports(statement, names) {
 ...
 */
export function tsModuleAliases(relPath) {
