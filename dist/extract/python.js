...
import { endLineOf, field, fieldText, firstChildOfType, lineOf, looksLikeConstant, namedChildren, signatureOf, splitQualified, stringLiteralDoc, } from './ast.js';
    ...
    switch (node.type) {
        ...
        case 'decorated_definition': {
            ...
            if (inner.type === 'function_definition')
                defineFunction(inner, scope, decorators);
            else if (inner.type === 'class_definition')
                defineClass(inner, scope, decorators);
            return;
        }
...
function defineFunction(node, scope, decorators) {
...
function defineClass(node, scope, decorators) {
    const name = fieldText(node, 'name');
    if (!name)
        return;
    const qualified = scope.prefix ? `${scope.prefix}.${name}` : name;
...
function recordPlainImport(node, scope) {
    ...
    for (const child of namedChildren(node)) {
        if (child.type === 'dotted_name') {
            ...
            if (module)
                scope.build.importEdge(scope.moduleNode, module, lineOf(node), { alias: alias ?? module });
        }
    }
}
function recordFromImport(node, scope) {
    ...
    for (const child of names) {
        if (child.type === 'dotted_name') {
            ...
            const alias = fieldText(child, 'alias');
            if (symbol)
                scope.build.importEdge(scope.moduleNode, module, line, { symbol, alias: alias ?? symbol });
        }
...
function recordDecorator(decorator, symbol, scope) {
 ...
 * are both one lookup.
 */
function defineEndpoint(callNode, decoratorName, handler, scope, line) {
...
function httpMethodFromArgs(args) {
    for (const arg of namedChildren(args)) {
        if (arg.type !== 'keyword_argument')
            continue;
        if (fieldText(arg, 'name') !== 'methods')
            continue;
...
function recordAssignment(node, scope) {
    ...
    if (left && left.type !== 'identifier')
        visitChildren(left, scope);
    if (right)
        visitNode(right, scope);
}
...
function recordTypeRefs(node, owner, scope) {
    ...
    while (stack.length > 0) {
        ...
        if (current.type === 'string') {
            ...
            if (/^[A-Z]\w*$/.test(inner))
                scope.build.ref(owner, 'references', inner, lineOf(current));
            continue;
        }
...
export function pythonModulePath(relPath) {
    let clean = relPath.replace(/\.pyi?$/, '');
    if (clean.endsWith('/__init__'))
        clean = clean.slice(0, -'/__init__'.length);
    if (clean === '__init__')
        clean = '';
