...
export const MARKDOWN_MARKERS = { start: MARKER_START, end: MARKER_END };
export const TOML_MARKERS = { start: TOML_MARKER_START, end: TOML_MARKER_END };
/**
 ...
 */
export function upsertBlock(existing, body, style = MARKDOWN_MARKERS) {
...
export function removeBlock(existing, style = MARKDOWN_MARKERS) {
    const startAt = existing.indexOf(style.start);
    const endAt = existing.indexOf(style.end);
    if (startAt === -1 || endAt <= startAt)
        return null;
...
export async function writeMarkedFile(file, body, style = MARKDOWN_MARKERS) {
...
export async function stripMarkedFile(file, style = MARKDOWN_MARKERS) {
    const existing = await readTextFileOrNull(file);
    if (existing === null)
        return false;
