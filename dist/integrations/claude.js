...
import { cliInvocation, emptyReport } from './types.js';
...
export const claudeIntegration = {
    ...
    async install(ctx) {
        ...
        if (await updateJsonFile(settingsFile, (root) => {
    ...
    async uninstall(repoRoot) {
        const report = emptyReport();
        if (await updateJsonFile(path.join(repoRoot, '.mcp.json'), (root) => {
        ...
        if (await stripMarkedFile(path.join(repoRoot, 'CLAUDE.md')))
            report.changed.push('CLAUDE.md');
        if (await updateJsonFile(path.join(repoRoot, '.claude', 'settings.json'), (root) => {
            const hooks = peekObject(root, 'hooks');
            if (!hooks || !Array.isArray(hooks.PostToolUse))
                return false;
            const kept = hooks.PostToolUse.filter((entry) => !isOurHook(entry));
