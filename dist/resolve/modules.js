...
import { applyAliases } from './tsconfig.js';
/**
 ...
 */
export function moduleCandidates(family, module, importerPath, facts) {
    switch (family) {
        case 'python':
...
function pythonCandidates(module, importerPath) {
    const leadingDots = /^\.+/.exec(module)?.[0].length ?? 0;
    if (leadingDots === 0)
        return dedupe([module, ...suffixes(module, '.')]);
...
function goCandidates(module, facts) {
    const out = [module];
    // An internal import is the go.mod module path plus the directory.
    if (facts.goModulePath) {
        if (module === facts.goModulePath)
            out.push('.');
        else if (module.startsWith(facts.goModulePath + '/'))
            out.push(module.slice(facts.goModulePath.length + 1));
    }
    // Without go.mod, or for a vendored path, the directory suffix still works.
    out.push(...suffixes(module, '/'));
    return dedupe(out);
}
function typescriptCandidates(module, importerPath, facts) {
 ...
 */
export function submoduleOf(family, module, symbol) {
    switch (family) {
        case 'python':
...
function javaCandidates(module) {
