...
import { moduleCandidates } from './modules.js';
 ...
 */
export function openLinks(config) {
    const out = [];
    for (const link of config.links) {
        ...
        try {
            ...
            log.warn(`could not read the index for ${link}: ${err.message}`);
        }
    }
    return out;
}
export function closeLinks(links) {
    for (const link of links)
        link.store.close();
}
function prefixesFor(facts) {
    const prefixes = [facts.goModulePath, facts.packageName].filter((p) => Boolean(p));
 ...
 */
export function resolveImportAcrossRepos(links, family, module, symbol) {
    for (const link of links) {
        const candidates = new Set();
        for (const prefix of link.packagePrefixes) {
            if (module === prefix)
                candidates.add('.');
            else if (module.startsWith(prefix + '/'))
                candidates.add(module.slice(prefix.length + 1));
            else if (module.startsWith(prefix + '.'))
                candidates.add(module.slice(prefix.length + 1));
        }
 ...
 */
export function resolveMemberAcrossRepos(link, filePath, name) {
 ...
 */
export function resolveQualifiedAcrossRepos(link, filePath, qualified) {
 ...
 */
export function resolveNameAcrossRepos(links, name) {
    const hits = [];
    for (const link of links) {
        for (const node of link.store.nodesByName(name)) {
            if (!node.exported || node.kind === 'module')
                continue;
            hits.push({ link, node });
        }
    }
