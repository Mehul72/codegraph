...
program
    ...
    .action(async (paths, options) => indexCommand(paths, options));
program
    .command('reindex')
...
program
    ...
    .argument('<files...>')
    .action(async (files) => touchCommand(files));
program
    .command('mcp')
...
program
    ...
    .argument('[agents...]')
    .action(async (agents) => installCommand(agents));
program
    .command('uninstall')
    .description('remove every config entry and instruction block codegraph added')
    .argument('[agents...]')
    .action(async (agents) => uninstallCommand(agents));
program
    .command('hook')
...
program
    ...
    .option(...budget)
    .action(async (query, options) => runQueryCommand('search_symbols', {
    query,
    kind: options.kind,
    lang: options.lang,
...
program
    ...
    .argument('<name>')
    .option(...budget)
    .action(async (name, options) => runQueryCommand('where_defined', { name, budget: numberOption(options.budget) }));
program
    .command('get-symbol')
    .alias('symbol')
    ...
    .argument('<name>')
    .option(...budget)
    .action(async (name, options) => runQueryCommand('get_symbol', { name, budget: numberOption(options.budget) }));
program
    ...
    .option(...budget)
    .action(async (symbol, options) => runQueryCommand('find_callers', {
    symbol,
...
program
    ...
    .option('-d, --depth <n>', 'hops to follow')
    .option(...budget)
    .action(async (symbol, options) => runQueryCommand('find_callees', {
    symbol,
    depth: numberOption(options.depth),
    budget: numberOption(options.budget),
}));
program
    ...
    .option('--no-cross-repo', 'ignore linked repos')
    .option(...budget)
    .action(async (target, options) => runQueryCommand('impact_of', {
    target,
    depth: numberOption(options.depth),
    cross_repo: options.crossRepo,
    budget: numberOption(options.budget),
...
program
    ...
    .argument('<b>')
    .option(...budget)
    .action(async (a, b, options) => runQueryCommand('shortest_path', { a, b, budget: numberOption(options.budget) }));
program
    .command('overview')
    .description('structural summary of the repo or a directory')
    .argument('[path]')
    .option(...budget)
    .action(async (dir, options) => runQueryCommand('overview', { path: dir, budget: numberOption(options.budget) }));
program
    ...
    .option(...budget)
    .action(async (ref, options) => runQueryCommand('changed_since', {
    ref,
    depth: numberOption(options.depth),
    budget: numberOption(options.budget),
}));
try {
    ...
    if (process.env.CODEGRAPH_LOG === 'debug')
