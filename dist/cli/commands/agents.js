...
import { cliInvocation } from '../../integrations/types.js';
export async function installCommand(names) {
...
export async function uninstallCommand(names) {
    const repoRoot = findRepoRoot();
    const out = (line) => process.stdout.write(line + '\n');
...
async function pickAgents(repoRoot, names) {
    if (names.length === 0 || names.includes('all'))
        return detectIntegrations(repoRoot);
    const picked = [];
...
export async function hookCommand(action) {
    const repoRoot = findRepoRoot();
    const out = (line) => process.stdout.write(line + '\n');
    if (action === 'install') {
        const server = await resolveServerCommand();
