 ...
 */
export async function touchCommand(files) {
    let session;
    try {
        session = await Session.open();
    }
    catch (err) {
        if (err instanceof NoIndexError)
            return;
        log.debug(`touch could not open the index: ${err.message}`);
        return;
    }
    try {
        ...
        log.debug(`touch failed: ${err.message}`);
    }
    finally {
        session.close();
    }
