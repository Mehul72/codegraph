...
import { detectIntegrations, integrationById, INTEGRATIONS, PACKAGE_SPEC, resolveServerCommand, } from '../../integrations/index.js';
...
export async function initCommand(options) {
    ...
    try {
        ...
        out(`index      ${formatCount(stats.filesIndexed)} ${plural(stats.filesIndexed, 'file')}, ${formatCount(stats.nodes)} ${plural(stats.nodes, 'symbol')}, ${formatCount(stats.edges)} ${plural(stats.edges, 'edge')} in ${formatDuration(stats.durationMs)}`);
 ...
 */
function pickExampleSymbol(store) {
    ...
    for (const node of store.allNodesLite()) {
        ...
        if (!best || degree > best.degree)
            best = { name: node.qualified ?? node.name, degree };
    }
    return best?.name ?? null;
}
async function wireUpAgents(repoRoot, requested, out) {
