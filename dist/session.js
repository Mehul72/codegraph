...
export class Session {
    ...
    constructor(repoRoot, config, store, pool, links, facts) {
        this.repoRoot = repoRoot;
        this.config = config;
        this.store = store;
        this.pool = pool;
    ...
    static async open(options = {}) {
        ...
        if ((options.requireIndex ?? true) && !pathExistsSync(file))
            throw new NoIndexError(repoRoot);
    ...
    async ensureFresh(options = {}) {
        ...
        if (Date.now() - this.lastFreshnessCheck < debounce)
            return null;
        ...
        this.freshnessInFlight = (async () => {
            try {
                ...
                log.warn(`freshness check failed (${err.message}), answering from the existing index`);
                return null;
            }
            finally {
