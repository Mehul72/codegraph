 ...
 */
export async function changedFilesSince(repoRoot, ref) {
    if (!isGitRepo(repoRoot))
        throw new GitError(`${repoRoot} is not a git repository`);
    const paths = new Set();
    for (const args of [
        ...
        for (const entry of output.split('\0')) {
            if (entry !== '')
                paths.add(entry.split(path.sep).join('/'));
        }
    }
    return [...paths].sort();
}
...
export async function describeHead(repoRoot) {
    if (!isGitRepo(repoRoot))
        return null;
    try {
