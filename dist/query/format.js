...
export function repoTag(node, localRepo) {
    return node.repo === localRepo ? '' : ` [repo:${node.repo}]`;
}
export function symbolHeadline(node, localRepo) {
...
export function detailLines(node) {
    const out = [];
    if (node.signature)
        out.push(`  sig  ${truncate(node.signature, 180)}`);
    if (node.doc)
        out.push(`  doc  ${truncate(node.doc, 160)}`);
    if (!node.exported)
        out.push('  not exported');
    return out;
}
/** One symbol per line, name padded so a list of them reads as columns. */
export function symbolLine(node, localRepo, nameWidth = 28) {
...
export function groupByFile(items) {
    ...
    for (const item of items) {
        ...
        if (list)
            ...
            groups.set(item.node.path, [item]);
    }
...
export function neighbourLine(item, localRepo, showDistance) {
...
export function confidenceSummary(items) {
    const counts = { exact: 0, resolved: 0, heuristic: 0 };
    for (const item of items)
        counts[item.edge.confidence]++;
    const parts = [];
    for (const key of ['exact', 'resolved', 'heuristic']) {
        if (counts[key] > 0)
...
export const CONFIDENCE_LEGEND = 'confidence: exact = seen in one AST, resolved = linked via imports, heuristic = name match only, verify before acting';
