...
const NO_TYPE_PARAMS: ReadonlySet<string> = new Set();
...
export const goExtractor: Extractor = {
  ...
  extract(input: ExtractInput) {
    ...
    const file: FileScope = { build, moduleNode, source, types: new Map(), methods: [] };
    try {
      for (const decl of namedChildren(root)) visitTopLevel(decl, file);
   ...
   */
  moduleAliases(relPath: string, source: string) {
    const aliases = new Set<string>();
    const packageName = packageNameFromSource(source);
    if (packageName) aliases.add(packageName);

    const dir = goPackageDir(relPath);
    if (dir !== '.') {
      const parts = dir.split('/').filter(Boolean);
      for (let i = 0; i < parts.length; i++) aliases.add(parts.slice(i).join('/'));
    }
    return [...aliases];
  },
};
...
interface PendingMethod {
  /** Receiver base type name, pointer and type arguments already stripped. */
  receiver: string;
  method: GraphNode;
  line: number;
}
...
interface FileScope {
  ...
  source: string;
  /** Types declared in this file, so a method can be hung off its receiver. */
  types: Map<string, GraphNode>;
  /** Methods parked until every type in the file has been seen. */
  methods: PendingMethod[];
}

function visitTopLevel(node: Node, file: FileScope): void {
  switch (node.type) {
    case 'package_clause':
      return;
...
function defineFunction(node: Node, file: FileScope): void {
  ...
  if (body) scanBody(body, symbol, file, typeParams);
}

function defineMethod(node: Node, file: FileScope): void {
  const name = fieldText(node, 'name');
  if (!name) return;
  ...
  file.build.edge(file.moduleNode, { kind: 'id', id: symbol.id }, 'defines', lineOf(node));
  if (receiver) file.methods.push({ receiver, method: symbol, line: lineOf(node) });
 ...
 */
function linkMethodsToReceivers(file: FileScope): void {
...
function defineTypes(node: Node, file: FileScope): void {
  for (const spec of namedChildren(node)) {
    if (spec.type === 'type_spec' || spec.type === 'type_alias') defineType(spec, file);
  }
}

function defineType(spec: Node, file: FileScope): void {
  ...
  if (underlying.type === 'struct_type') recordStructShape(underlying, symbol, file, typeParams);
  else if (underlying.type === 'interface_type') recordInterfaceShape(underlying, symbol, file, typeParams);
  else recordTypeRefs(underlying, symbol, file, typeParams);
}
 ...
 */
function kindOfType(underlying: Node | null): NodeKind {
  return underlying?.type === 'interface_type' ? 'interface' : 'struct';
}

function typeSignature(spec: Node, underlying: Node | null): string {
  const name = fieldText(spec, 'name') ?? '';
...
function shapeLabel(underlying: Node | null): string {
  if (!underlying) return '';
  if (underlying.type === 'struct_type') return 'struct';
  if (underlying.type === 'interface_type') return 'interface';
  return underlying.text;
}

function recordStructShape(struct: Node, owner: GraphNode, file: FileScope, typeParams: ReadonlySet<string>): void {
  ...
  for (const decl of childrenOfType(list, 'field_declaration')) {
    ...
    if (!field(decl, 'name')) {
      ...
      if (embedded) file.build.ref(owner, 'inherits', embedded.name, lineOf(decl), embedded.qualifier);
      continue;
    }
...
function recordInterfaceShape(iface: Node, owner: GraphNode, file: FileScope, typeParams: ReadonlySet<string>): void {
  for (const elem of namedChildren(iface)) {
    ...
    if (embedded) file.build.ref(owner, 'inherits', embedded.name, lineOf(elem), embedded.qualifier);
  }
}

function defineInterfaceMethod(
  elem: Node,
  iface: GraphNode,
  file: FileScope,
 ...
 */
function defineValues(node: Node, file: FileScope): void {
  ...
  for (const spec of valueSpecs(node, specType)) {
    const declared: GraphNode[] = [];

    for (const nameNode of fieldNodes(spec, 'name')) {
      const name = nameNode.text;
      if (name === '_' || !(isExported(name) || looksLikeConstant(name))) continue;
    ...
    if (type) recordTypeRefs(type, owner, file, NO_TYPE_PARAMS);
    const value = field(spec, 'value');
    if (value) scanBody(value, owner, file, NO_TYPE_PARAMS);
  }
}

/** A parenthesized `var (...)` block wraps its specs in a list, `const (...)` does not. */
function valueSpecs(node: Node, specType: string): Node[] {
  const specs: Node[] = [];
  for (const child of namedChildren(node)) {
    if (child.type === specType) specs.push(child);
    else if (child.type === `${specType}_list`) specs.push(...childrenOfType(child, specType));
  }
  return specs;
}

function recordImports(node: Node, file: FileScope): void {
...
function importSpecs(node: Node): Node[] {
  const specs: Node[] = [];
  for (const child of namedChildren(node)) {
    if (child.type === 'import_spec') specs.push(child);
    else if (child.type === 'import_spec_list') specs.push(...childrenOfType(child, 'import_spec'));
  }
  return specs;
}
 ...
 */
function scanBody(root: Node, owner: GraphNode, file: FileScope, typeParams: ReadonlySet<string>): void {
  walk(root, (node) => {
    switch (node.type) {
      ...
      case 'type_conversion_expression': {
        ...
        if (type) recordTypeRefs(type, owner, file, typeParams);
        return true;
      }
...
function recordCall(node: Node, owner: GraphNode, file: FileScope): void {
  ...
  if (target.type === 'identifier') {
    ...
    if (!PREDECLARED_TYPES.has(target.text)) file.build.ref(owner, 'calls', target.text, line);
    return;
  }
  ...
  if (target.type === 'selector_expression') {
    ...
    if (name) file.build.ref(owner, 'calls', name, line, callQualifier(field(target, 'operand')));
    return;
  }

  if (target.type === 'generic_type') {
    const instantiated = baseTypeRef(target);
    if (instantiated) file.build.ref(owner, 'calls', instantiated.name, line, instantiated.qualifier);
    return;
  }
 ...
 */
function callQualifier(operand: Node | null): string | null {
  let current = operand;
  for (let depth = 0; current && depth < MAX_UNWRAP_DEPTH; depth++) {
    if (current.type === 'identifier') return operand?.text ?? null;
...
function recordSqlStrings(node: Node, owner: GraphNode, file: FileScope): void {
...
function recordSignatureTypes(
  ...
  if (params) recordTypeRefs(params, owner, file, typeParams);
  const result = field(node, 'result');
  if (result) recordTypeRefs(result, owner, file, typeParams);
}
 ...
 * on Order.
 */
function recordTypeRefs(node: Node, owner: GraphNode, file: FileScope, typeParams: ReadonlySet<string>): void {
  walk(node, (current) => {
    if (current.type === 'qualified_type') {
      const name = fieldText(current, 'name');
      if (name) file.build.ref(owner, 'references', name, lineOf(current), fieldText(current, 'package'));
      return false;
    }
    if (current.type === 'type_identifier') {
      const name = current.text;
      const worthEdge = /^[A-Za-z]/.test(name) && !PREDECLARED_TYPES.has(name) && !typeParams.has(name);
      if (worthEdge) file.build.ref(owner, 'references', name, lineOf(current));
      return false;
    }
    return true;
  });
 ...
 */
function typeParamNames(node: Node): ReadonlySet<string> {
  const names = new Set<string>();

  const list = field(node, 'type_parameters');
  if (list) {
    for (const decl of childrenOfType(list, 'type_parameter_declaration')) {
      for (const nameNode of fieldNodes(decl, 'name')) names.add(nameNode.text);
    }
  }
  ...
  if (receiver) {
    walk(receiver, (current) => {
      if (current.type !== 'type_arguments') return true;
      for (const elem of childrenOfType(current, 'type_elem')) {
        const inner = namedChildren(elem)[0];
        if (inner?.type === 'type_identifier') names.add(inner.text);
      }
      return false;
    });
  }
...
function receiverTypeName(receiver: Node | null): string | null {
...
function baseTypeRef(type: Node | null): TypeRef | null {
  let current = type;
  for (let depth = 0; current && depth < MAX_UNWRAP_DEPTH; depth++) {
    switch (current.type) {
...
export function goPackageDir(relPath: string): string {
