...
const MAPPING_VERBS: Record<string, string | null> = {
  getmapping: 'GET',
  postmapping: 'POST',
  putmapping: 'PUT',
  patchmapping: 'PATCH',
...
export const javaExtractor: Extractor = {
  ...
  modulePath(relPath: string, source: string) {
...
interface Scope {
  ...
  owner: GraphNode;
  /** Dotted prefix for qualified names, empty at file level. */
  prefix: string;
  /** Set inside an interface body, where members are public by rule. */
  inInterface: boolean;
  /** Type variables in scope, which are declarations rather than references. */
  typeVars: ReadonlySet<string>;
  /** Class level @RequestMapping path, prefixed onto every handler below it. */
  basePath: string;
}
...
function defineType(node: Node, scope: Scope): void {
  ...
  if (typeParams) recordTypeRefs(typeParams, symbol, inner);

  const superclass = field(node, 'superclass');
  if (superclass) recordSupertypes(superclass, symbol, inner, 'inherits');
  const implemented = field(node, 'interfaces');
  if (implemented) recordSupertypes(implemented, symbol, inner, 'implements');
  ...
  if (extended) recordSupertypes(extended, symbol, inner, 'inherits');

  // Record components are parameters and field types at the same time.
  const components = field(node, 'parameters');
  if (components) recordTypeRefs(components, symbol, inner);

  const body = field(node, 'body');
  if (body) visitChildren(body, inner);
}
...
function defineMethod(node: Node, scope: Scope): void {
  ...
  const body = field(node, 'body');
  if (body) visitChildren(body, { ...inner, owner: symbol, prefix: qualified });
}

function defineConstructor(node: Node, scope: Scope): void {
  const name = fieldText(node, 'name');
  if (!name) return;
  ...
  const body = field(node, 'body');
  if (body) visitChildren(body, { ...scope, owner: symbol, prefix: qualified });
}

/** Return type, parameters and thrown exceptions, which read the same way. */
function recordMemberTypes(node: Node, symbol: GraphNode, scope: Scope): void {
  const typeParams = firstChildOfType(node, 'type_parameters');
  if (typeParams) recordTypeRefs(typeParams, symbol, scope);
  ...
  if (thrown) recordTypeRefs(thrown, symbol, scope);
}

function defineField(node: Node, scope: Scope): void {
  ...
  if (type) recordTypeRefs(type, scope.owner, scope);
  ...
  for (const declarator of childrenOfType(node, 'variable_declarator')) {
    ...
    if (value) visitNode(value, { ...scope, owner });
  }
}
...
function recordSupertypes(node: Node, symbol: GraphNode, scope: Scope, type: 'inherits' | 'implements'): void {
  ...
  for (const entry of namedChildren(list ?? node)) {
    ...
    if (name === '' || !/^[A-Z]/.test(name) || scope.typeVars.has(name)) continue;
    ...
    if (args) recordTypeRefs(args, symbol, scope);
  }
}

function recordAnnotationRefs(node: Node, owner: GraphNode, scope: Scope): void {
  for (const annotation of annotationsOf(node)) {
    const name = annotationName(annotation);
    if (name !== '') scope.build.ref(owner, 'references', name, lineOf(annotation));
    ...
    if (args) {
      walk(args, (child) => {
        if (child.type === 'string_literal') recordSqlStrings(child, owner, scope);
      });
    }
  }
}
 ...
 */
function defineEndpoints(node: Node, handler: GraphNode, scope: Scope): void {
  for (const annotation of annotationsOf(node)) {
...
function classRoutePrefix(node: Node): string {
  for (const annotation of annotationsOf(node)) {
    if (annotationName(annotation).toLowerCase() !== 'requestmapping') continue;
    const args = field(annotation, 'arguments');
    if (args) return routeFromArgs(args);
  }
  return '';
}
 ...
 */
function routeFromArgs(args: Node): string {
  for (const arg of namedChildren(args)) {
    if (arg.type === 'element_value_pair') {
      const key = fieldText(arg, 'key');
      if (key !== 'value' && key !== 'path') continue;
...
function methodFromArgs(args: Node): string {
  for (const arg of namedChildren(args)) {
    if (arg.type !== 'element_value_pair' || fieldText(arg, 'key') !== 'method') continue;
    const value = field(arg, 'value');
    if (!value) continue;
    ...
    if (verbs.length > 0) return verbs.join('|');
  }
  return 'ANY';
}

function firstStringIn(node: Node): string | null {
  if (node.type === 'string_literal') return unquote(node.text).trim();
...
function recordSqlStrings(node: Node, owner: GraphNode, scope: Scope): void {
...
function recordTypeRefs(node: Node, owner: GraphNode, scope: Scope): void {
  walk(node, (current) => {
    if (current.type !== 'type_identifier') return;
    const name = current.text;
    if (!/^[A-Z]/.test(name) || scope.typeVars.has(name)) return;
    scope.build.ref(owner, 'references', name, lineOf(current));
  });
}

/** `java.util.Map.Entry<String, Order>` -> `Entry`. */
function baseTypeName(node: Node): string {
...
function isExported(node: Node, scope: Scope): boolean {
  ...
  if (mods.has('public') || mods.has('protected')) return true;
...
function modifiersOf(node: Node): Set<string> {
  ...
  if (!mods) return found;
  for (const child of mods.children) {
    if (child && !child.isNamed) found.add(child.type);
  }
  return found;
}

function annotationsOf(node: Node): Node[] {
  const mods = firstChildOfType(node, 'modifiers');
...
function annotationName(annotation: Node): string {
...
function packageFromSource(source: string): string | null {
...
function typeNameFromPath(relPath: string): string {
