 ...
 */
export function tablesInSql(text: string): string[] {
  ...
  for (const pattern of TABLE_PATTERNS) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const bare = (cleaned.includes('.') ? (cleaned.split('.').pop() as string) : cleaned).toLowerCase();
      if (bare.length < 2 || NOT_TABLES.has(bare) || seen.has(bare)) continue;
      seen.add(bare);
      found.push(bare);
    }
  }
