...
const TYPE_KINDS: Record<string, NodeKind> = {
  class: 'class',
  struct: 'struct',
  protocol: 'interface',
...
const NO_VARIABLES: ReadonlyMap<string, string | null> = new Map();
...
export const swiftExtractor: Extractor = {
   ...
   */
  prepareSource(source: string) {
    return source.replace(TRY_AWAIT, (_match, tryKeyword: string) => tryKeyword + ' '.repeat('await'.length));
  },
...
/** What the whole file knows before any one declaration is visited. */
interface FileScope {
  build: SymbolBuilder;
  moduleNode: GraphNode;
  source: string;
  /** Modules this file imports. In `Store.Order`, the type is Order. */
  importedModules: ReadonlySet<string>;
  /**
   * The declared type of every property, per declaring type, with '' for the
   * file's top-level variables. Collected first, because a property is often
   * declared below the method that uses it.
   */
  propertyTypes: ReadonlyMap<string, ReadonlyMap<string, string | null>>;
  /** Types declared in this file, so an extension here can hang members off them. */
  declaredTypes: Map<string, GraphNode>;
  /** Extension members and conformances, parked until every type in the file is known. */
  extensionMembers: PendingMember[];
  extensionConformances: PendingConformance[];
}

interface Scope {
  file: FileScope;
  /** Nearest enclosing symbol, which owns any reference found. */
  owner: GraphNode;
  /** Qualified name of the enclosing or extended type, null at file level. */
  typeName: string | null;
  /** Set inside an extension, whose members also belong to the type it extends. */
  extensionOf: string | null;
  /** Whether a declaration that names no access level is exported. */
  exportedByDefault: boolean;
  /** Generic parameters and associated types, which are declared rather than referenced. */
  typeParams: ReadonlySet<string>;
  /**
   * Variables in scope, mapped to their declared type, or to null when the
   * source does not say. A bare call to one of these runs a closure.
   */
...
interface PendingConformance extends TypeRef {
  typeName: string;
  line: number;
}

/** One name bound by a `let` or `var`, with the parts that belong to it. */
...
function visitDeclarations(container: Node, scope: Scope): void {
  for (const node of namedChildren(container)) visitDeclaration(node, scope);
}

function visitDeclaration(node: Node, scope: Scope): void {
  switch (node.type) {
    ...
    case 'class_declaration':
      if (declarationKind(node) === 'extension') defineExtension(node, scope);
      else defineType(node, scope);
      return;
...
function defineType(node: Node, scope: Scope): void {
  ...
  if (body) visitDeclarations(body, inner);
}
 ...
 */
function defineExtension(node: Node, scope: Scope): void {
 ...
 */
function defineFunction(node: Node, name: string | null, scope: Scope): void {
  if (!name) return;
  ...
  for (const part of namedChildren(node)) {
    if (part.type !== 'modifiers' && !BODY_TYPES.has(part.type)) scan(part, inner);
  }
  if (body) scan(body, inner);
}
 ...
 */
function defineProperty(node: Node, scope: Scope): void {
  ...
  for (const binding of propertyBindings(node)) {
    if (binding.getter) {
      ...
      symbols.push(symbol);
      const getterScope: Scope = { ...scope, owner: symbol, variables: withLocals(scope, node, binding.getter) };
...
function defineTypeAlias(node: Node, scope: Scope): void {
  ...
  const inner: Scope = { ...scope, owner: symbol, typeParams: withTypeParams(scope.typeParams, node) };
 ...
 */
function defineSymbol(node: Node, scope: Scope, spec: { name: string; kind: NodeKind }): GraphNode {
 ...
 */
function linkExtensions(file: FileScope): void {
  for (const pending of file.extensionMembers) {
    const type = file.declaredTypes.get(pending.typeName);
    if (type) file.build.edge(type, { kind: 'id', id: pending.member.id }, 'defines', pending.line);
  }
 ...
 */
function conformanceEdge(keyword: string, index: number): EdgeType {
  if (keyword === 'protocol') return 'inherits';
  return keyword === 'class' && index === 0 ? 'inherits' : 'implements';
}

function recordConformances(
  node: Node,
  scope: Scope,
  record: (ref: TypeRef, index: number, line: number) => void,
): void {
  childrenOfType(node, 'inheritance_specifier').forEach((spec, index) => {
    ...
    if (ref) record(ref, index, lineOf(spec));
    // Generic arguments on a supertype are ordinary references: Repository<Order>.
    for (const args of childrenOfType(type, 'type_arguments')) scan(args, scope);
  });
}

/** Constraints in `<T: OrderStore>` and in a where clause are dependencies of the declaration. */
function recordGenericConstraints(node: Node, scope: Scope): void {
 ...
 * to the compiler.
 */
function recordAttributes(node: Node, owner: GraphNode, scope: Scope): void {
...
function recordImport(node: Node, scope: Scope): void {
  ...
  if (module) scope.file.build.importEdge(scope.file.moduleNode, module, lineOf(node), { alias: module });
}

function scanAll(nodes: ReadonlyArray<Node | null>, scope: Scope): void {
  for (const node of nodes) {
    if (node) scan(node, scope);
  }
}
 ...
 */
function scan(root: Node, scope: Scope): void {
  walk(root, (node) => {
    switch (node.type) {
      ...
      case 'constructor_expression':
        ...
        for (const part of namedChildren(node)) {
          if (part.type === 'user_type') scanAll(childrenOfType(part, 'type_arguments'), scope);
          else scan(part, scope);
        }
        return false;
...
function recordCall(node: Node, scope: Scope): void {
  ...
  if (callee.type === 'simple_identifier') {
    ...
    const skip =
      scope.variables.has(name) || scope.typeParams.has(name) || BUILTIN_FUNCTIONS.has(name) || BUILTIN_TYPES.has(name);
    if (!skip) scope.file.build.ref(scope.owner, 'calls', name, line);
    return;
  }
  ...
  if (name === 'init') {
    ...
    if (type) recordTypeCall(type, line, scope);
    return;
  }
  ...
  if (receiver !== null) scope.file.build.ref(scope.owner, 'calls', name, line, receiver);
}

/** `Box<Int>(value: 1)` is how a generic type is built. */
function recordConstruction(node: Node, scope: Scope): void {
  const type = field(node, 'constructed_type');
  const typeName = type ? typeChainName(type, scope.file.importedModules) : null;
  if (typeName) recordTypeCall(typeName, lineOf(node), scope);
}

/** Running an initializer is a call on the type, as Java records `new Foo()`. */
function recordTypeCall(typeName: string, line: number, scope: Scope): void {
  const cut = typeName.lastIndexOf('.');
  const name = typeName.slice(cut + 1);
  const qualifier = cut === -1 ? null : typeName.slice(0, cut);
  if (qualifier === null && (BUILTIN_TYPES.has(name) || scope.typeParams.has(name))) return;
  scope.file.build.ref(scope.owner, 'calls', name, line, qualifier);
}

function recordTypeRef(node: Node, scope: Scope): void {
  const ref = typeRefOf(node, scope);
  if (ref) scope.file.build.ref(scope.owner, 'references', ref.name, lineOf(node), ref.qualifier);
}

/** `decoder.decode(Order.self, ...)` depends on Order as surely as a signature naming it does. */
function recordMetatypeRef(node: Node, scope: Scope): void {
  if (memberName(node) !== 'self') return;
  const target = field(node, 'target');
  const chain = target ? typeCallChain(target, 0) : null;
  const head = chain?.[0];
  const name = chain?.[chain.length - 1];
  if (!chain || !head || !name || BUILTIN_TYPES.has(head) || scope.typeParams.has(head)) return;
 ...
 */
function receiverType(node: Node, scope: Scope, depth: number): string | null {
...
function namedReceiverType(name: string, scope: Scope): string | null {
  // `Self` is the enclosing type, which resolution treats just like self.
  if (name === 'Self') return 'self';
  if (scope.variables.has(name)) return scope.variables.get(name) ?? null;
...
function memberReceiverType(node: Node, scope: Scope, depth: number): string | null {
  const member = memberName(node);
  const target = field(node, 'target');
  if (!member || !target) return null;

  const base = receiverType(target, scope, depth + 1);
  if (base === null || base === 'super') return null;
...
function constructionType(node: Node, modules: ReadonlySet<string>): string | null {
...
function typeNamedBy(callee: Node, modules: ReadonlySet<string>): string | null {
  const chain = typeCallChain(callee, 0);
  if (!chain) return null;
  if (chain.length > 1 && modules.has(chain[0] as string)) chain.shift();
  return chain.join('.');
}

function typeCallChain(node: Node, depth: number): string[] | null {
  if (depth >= MAX_CHAIN_DEPTH) return null;
  if (node.type === 'simple_identifier') return isTypeLike(node.text) ? [node.text] : null;
  if (node.type !== 'navigation_expression') return null;

  const member = memberName(node);
  const target = field(node, 'target');
  const base = member && target ? typeCallChain(target, depth + 1) : null;
  if (!member || !base) return null;
  if (member === 'init') return base;
  return isTypeLike(member) ? [...base, member] : null;
}

function memberName(navigation: Node): string | null {
 ...
 * left out.
 */
function typeRefOf(node: Node, scope: Scope): TypeRef | null {
  if (node.type !== 'user_type') return null;
  const chain = typeChain(node);
  const head = chain[0];
  const name = chain[chain.length - 1];
  if (!head || !name || BUILTIN_TYPES.has(head) || scope.typeParams.has(head)) return null;
...
function typeChain(type: Node): string[] {
  const chain = childrenOfType(type, 'type_identifier').map((part) => part.text);
  if (chain.length > 1 && METATYPE_SUFFIXES.has(chain[chain.length - 1] as string)) chain.pop();
  return chain;
}

/** A user type's name as this file's module would qualify it: `Store.Order` is Order. */
function typeChainName(type: Node, modules: ReadonlySet<string>): string | null {
 ...
 */
function declaredTypeName(type: Node | null, modules: ReadonlySet<string>): string | null {
  let current = type;
  for (let depth = 0; current && depth < MAX_CHAIN_DEPTH; depth++) {
    switch (current.type) {
      case 'user_type':
...
function bindingType(binding: PropertyBinding, modules: ReadonlySet<string>): string | null {
  if (binding.type) return declaredTypeName(binding.type, modules);
 ...
 */
function propertyBindings(decl: Node): PropertyBinding[] {
  const bindings: PropertyBinding[] = [];
  let current: PropertyBinding | null = null;
  ...
  for (let i = 0; i < decl.childCount; i++) {
    ...
    if (child.type === 'type_annotation') current.type = field(child, 'name');
    else if (fieldName === 'value') current.value = child;
    else if (fieldName === 'computed_value') current.getter = child;
    else if (child.type === 'willset_didset_block') current.observers = child;
  }
  return bindings;
}
...
function collectPropertyTypes(
  ...
  const byType = new Map<string, Map<string, string | null>>();

  const collect = (container: Node, typeName: string): void => {
    ...
    for (const member of namedChildren(container)) {
      ...
      if (member.type !== 'class_declaration' && member.type !== 'protocol_declaration') continue;
      const nestedName = nestedTypeName(member, typeName, modules);
      const body = field(member, 'body');
      if (nestedName && body) collect(body, nestedName);
    }
  };
...
function nestedTypeName(decl: Node, outer: string, modules: ReadonlySet<string>): string | null {
...
function variablesOf(file: FileScope, typeName: string): ReadonlyMap<string, string | null> {
  const globals = file.propertyTypes.get('') ?? NO_VARIABLES;
  const own = file.propertyTypes.get(typeName);
  if (!own || own.size === 0) return globals;
 ...
 */
function withLocals(scope: Scope, declaration: Node, body: Node | null): ReadonlyMap<string, string | null> {
  ...
  for (const param of childrenOfType(declaration, 'parameter')) {
    ...
    if (nameNode) variables.set(nameNode.text, declaredTypeName(typeNode ?? null, scope.file.importedModules));
  }

  if (!body) return variables;
  const locals = new Map<string, string | null>();
  walk(body, (node) => {
    if (node.type !== 'property_declaration') return true;
    for (const binding of propertyBindings(node)) {
      addVariable(locals, binding.name, bindingType(binding, scope.file.importedModules));
    }
    return true;
  });
  for (const [name, type] of locals) variables.set(name, type);
  return variables;
}

function addVariable(variables: Map<string, string | null>, name: string, type: string | null): void {
  if (variables.has(name) && variables.get(name) !== type) variables.set(name, null);
  else variables.set(name, type);
}

/** Generic parameters declared on `node`, plus any extra names, added to those already in scope. */
function withTypeParams(current: ReadonlySet<string>, node: Node, extra: readonly string[] = []): ReadonlySet<string> {
...
function importedModuleNames(root: Node): Set<string> {
...
function importedModule(decl: Node): string | null {
 ...
 */
function isExported(node: Node, scope: Scope): boolean {
  const level = accessLevel(node);
  if (level === null) return scope.exportedByDefault;
  return level === 'public' || level === 'open';
}

function accessLevel(node: Node): string | null {
  const modifiers = firstChildOfType(node, 'modifiers');
  ...
  for (const modifier of childrenOfType(modifiers, 'visibility_modifier')) {
    ...
    if (!narrowsSetter) return modifier.text;
  }
  return null;
}
...
function mutability(node: Node): string | null {
...
function fieldNodes(node: Node, name: string): Node[] {
  const found: Node[] = [];
 ...
 */
export function swiftModuleOf(relPath: string): string {
  const dirs = relPath.replace(/\\/g, '/').split('/').slice(0, -1);
  for (let i = dirs.length - 2; i >= 0; i--) {
    if (TARGET_ROOTS.has(dirs[i] as string)) return dirs[i + 1] as string;
  }
  return dirs[0] ?? '.';
}
