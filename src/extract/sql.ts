/**
 * The only extractor without a grammar. There is no maintained prebuilt
 * tree-sitter SQL wasm, and compiling one natively breaks `npm install` on
 * some machines, which the project rules out. DDL is regular enough that a
 * scanner over masked text gets the schema shape (tables, views, routines,
 * foreign keys) without a parser, so this file keeps `grammar: null` and
 * `input.tree` arrives as null.
 *
 * Every SQL name is stored lower-cased. See bareName for why that matters.
 */
...
export const sqlExtractor: Extractor = {
  ...
  extract(input: ExtractInput) {
    ...
    const scan: Scan = { build, moduleNode, lines: source.split('\n'), lineStarts, declared: new Map() };
    ...
    for (const statement of statements) declare(statement, scan);
    for (const statement of statements) relate(statement, scan);
    return build.result();
  },
};
...
interface Scan {
  ...
  /** Unmasked source lines, for the comment above a statement. */
  lines: readonly string[];
  lineStarts: readonly number[];
  /** Tables and views declared in this file, by bare name. */
  declared: Map<string, GraphNode>;
}

interface Statement {
  /** Offsets into the source, so every line number is the real one. */
  start: number;
  end: number;
  /** The masked slice, safe to scan for keywords. */
  text: string;
}

function declare(statement: Statement, scan: Scan): void {
...
function define(statement: Statement, scan: Scan, name: string, kind: 'table' | 'function', keyword: string): void {
  if (name === '') return;
  ...
  if (!scan.declared.has(name)) scan.declared.set(name, node);
}

function relate(statement: Statement, scan: Scan): void {
  const table = CREATE_TABLE.exec(statement.text);
  if (table) {
    const node = scan.declared.get(bareName(table[1]));
    if (node) recordForeignKeys(statement, node, scan);
    return;
  }
  ...
  if (view) {
    ...
    if (node) recordViewQueries(statement, view[0].length, node, scan);
    return;
  }
  ...
  if (index) {
    ...
    if (target !== '') scan.build.ref(scan.moduleNode, 'references', target, lineAt(scan.lineStarts, statement.start));
  }
}

function recordForeignKeys(statement: Statement, from: GraphNode, scan: Scan): void {
 ...
 */
function recordViewQueries(statement: Statement, headLength: number, view: GraphNode, scan: Scan): void {
  const line = lineAt(scan.lineStarts, statement.start);
...
function splitStatements(masked: string): Statement[] {
  ...
  const push = (from: number, to: number): void => {
    let head = from;
    let tail = to;
    while (head < tail && isSpace(masked[head])) head++;
    while (tail > head && isSpace(masked[tail - 1])) tail--;
    if (tail > head) statements.push({ start: head, end: tail, text: masked.slice(head, tail) });
  };
 ...
 */
function bareName(raw: string | undefined): string {
  if (!raw) return '';
  return raw.replace(/[`"[\]]/g, '').trim().toLowerCase();
}

/** The `--` comment block directly above a statement, first line only. */
function leadingComment(scan: Scan, line: number): string | null {
