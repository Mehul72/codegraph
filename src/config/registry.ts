...
const EMPTY: Registry = { version: 1, repos: [] };

export async function loadRegistry(): Promise<Registry> {
  const text = await readTextFileOrNull(registryPath());
  if (text === null) return { ...EMPTY, repos: [] };
  try {
    const parsed = JSON.parse(text) as Partial<Registry>;
    const repos = Array.isArray(parsed.repos) ? parsed.repos : [];
    return {
      version: typeof parsed.version === 'number' ? parsed.version : 1,
      repos: repos.filter((r): r is RegistryEntry => Boolean(r && typeof r.root === 'string' && typeof r.name === 'string')),
    };
  } catch (err) {
    log.warn(`global registry is unreadable (${(err as Error).message}), treating it as empty`);
    return { ...EMPTY, repos: [] };
  }
}
 ...
 */
export async function registerRepo(name: string, root: string): Promise<void> {
  ...
  try {
    ...
    log.warn(`could not record this repo in ${registryPath()} (${(err as Error).message}), cross-repo queries will not see it`);
  }
}
 ...
 */
export async function resolveRepoRef(ref: string): Promise<RegistryEntry | null> {
  const registry = await loadRegistry();
