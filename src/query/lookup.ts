 ...
 */
export function lookupSymbol(store: Store, query: string): SymbolLookup {
  const trimmed = query.trim();
  if (trimmed === '') return { matches: [], how: 'none' };
  ...
  if (trimmed.split(':').length >= 4) {
    ...
    if (node) return { matches: [node], how: 'id' };
  }

  // file.py:name, which is how our own output points at things.
  const filePair = /^(.+\.[a-zA-Z]+):([A-Za-z_][\w.]*)$/.exec(trimmed);
  if (filePair) {
    const [, file, name] = filePair as unknown as [string, string, string];
    const inFile = store.nodesInFile(file).filter((n) => n.name === name || n.qualified === name);
    if (inFile.length > 0) return { matches: sortMatches(inFile), how: 'in-file' };
  }

  const byQualified = store.nodesByQualified(trimmed);
  if (byQualified.length > 0) return { matches: sortMatches(byQualified), how: 'qualified' };

  const byName = store.nodesByName(trimmed);
  if (byName.length > 0) return { matches: sortMatches(byName), how: 'name' };
  ...
  if (trimmed.includes('.')) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      const candidates = [...store.nodesByQualified(tail), ...store.nodesByName(tail)].filter((node) =>
        moduleMatches(store, node.path, prefix),
      );
      if (candidates.length > 0) return { matches: sortMatches(dedupeById(candidates)), how: 'qualified' };
    }
    const last = parts[parts.length - 1] as string;
    const byLast = store.nodesByName(last);
    if (byLast.length > 0) return { matches: sortMatches(byLast), how: 'name' };
  }

  const insensitive = store.nodesByNameLower(trimmed);
  if (insensitive.length > 0) return { matches: sortMatches(insensitive), how: 'name' };

  const fuzzy = fuzzySearch(store, trimmed, 12);
  return fuzzy.length > 0 ? { matches: fuzzy, how: 'fuzzy' } : { matches: [], how: 'none' };
}

function moduleMatches(store: Store, filePath: string, prefix: string): boolean {
...
export function fuzzySearch(store: Store, query: string, limit: number, filters: SearchFilters = {}): GraphNode[] {
  const pool = store.searchCandidates(query, Math.max(limit * 30, 400));
  const scored: Array<{ node: GraphNode; score: number }> = [];

  for (const node of pool) {
    if (filters.kind && node.kind !== filters.kind) continue;
    if (filters.lang && node.lang !== filters.lang) continue;
...
export function sortMatches(nodes: readonly GraphNode[]): GraphNode[] {
...
function compareLocation(a: GraphNode, b: GraphNode): number {
  if (a.path !== b.path) return a.path < b.path ? -1 : 1;
  return a.lineStart - b.lineStart;
}

function dedupeById(nodes: readonly GraphNode[]): GraphNode[] {
