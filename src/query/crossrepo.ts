 ...
 * database.
 */
export async function foreignCallers(selfRoot: string, ids: readonly string[]): Promise<ForeignEdge[]> {
  ...
  for (const entry of registry.repos) {
    if (path.resolve(entry.root) === selfAbs) continue;
    const file = dbPath(entry.root);
    if (!pathExistsSync(file)) continue;
    ...
    try {
      ...
      for (const edge of edges) {
        ...
        if (node) out.push({ repo: entry.name, node, edge });
      }
    } catch (err) {
      log.debug(`skipping linked repo ${entry.name}: ${(err as Error).message}`);
    } finally {
      store?.close();
    }
  }
