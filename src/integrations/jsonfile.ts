 ...
 */
export async function updateJsonFile(file: string, mutate: (root: JsonObject) => boolean): Promise<boolean> {
  ...
  if (existing !== null && existing.trim() !== '') {
    try {
      ...
      log.warn(`${file} is not valid JSON (${(err as Error).message}), leaving it alone`);
      return false;
    }
  }
...
export function objectAt(root: JsonObject, ...keys: string[]): JsonObject {
  let cursor = root;
...
export function peekObject(root: JsonObject, ...keys: string[]): JsonObject | null {
  let cursor: unknown = root;
  for (const key of keys) {
    if (!cursor || typeof cursor !== 'object' || Array.isArray(cursor)) return null;
    cursor = (cursor as JsonObject)[key];
  }
  return cursor && typeof cursor === 'object' && !Array.isArray(cursor) ? (cursor as JsonObject) : null;
}

/** Drop empty objects we created, so uninstall leaves no residue. */
export function pruneEmpty(root: JsonObject, ...keys: string[]): void {
  for (let depth = keys.length; depth > 0; depth--) {
    const parentKeys = keys.slice(0, depth - 1);
    const leaf = keys[depth - 1] as string;
