...
export async function statusCommand(): Promise<void> {
  ...
  try {
    ...
    if (indexedAt) out(`updated   ${indexedAt.toISOString().replace('T', ' ').slice(0, 19)}`);
    const head = await describeHead(session.repoRoot);
    if (head) out(`git       ${head}`);

    out(
      `freshness ${formatDuration(freshnessMs)} to check${refresh && refresh.filesIndexed > 0 ? `, reindexed ${refresh.filesIndexed} changed files` : ', nothing changed'}`,
    );
...
function wiredMarker(repoRoot: string, agentId: string): string {
