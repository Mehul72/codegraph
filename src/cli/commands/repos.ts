...
export async function reposCommand(): Promise<void> {
  ...
  for (const entry of registry.repos) {
    ...
    out(
      `${pad(entry.name + marker, 22)} ${pad(stats.symbols === null ? 'missing' : formatCount(stats.symbols), 9)} ${pad(stats.indexedAt ?? 'never', 20)} ${entry.root}`,
    );
  }
  out('');
...
function readStats(file: string): { symbols: number | null; indexedAt: string | null } {
  if (!pathExistsSync(file)) return { symbols: null, indexedAt: null };
